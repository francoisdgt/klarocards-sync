jQuery(function ($) {
  ...
  $("#retrieve-cards-btn").on("click", function (e) {
    ...
    $.post(
      ...
      function (response) {
        if (response.success) {
          ...
          const successMessage =
            response.data || "Synchronisation terminée avec succès !";
          ...
          const notice = $(
            '<div class="notice notice-error is-dismissible"><p><strong>Erreur :</strong> ' +
              errorMessage +
              "</p></div>"
          );
          $("#retrieve-cards-btn").after(notice);
      ...
      const errorMessage =
        "Erreur de connexion : " + (error || "Problème réseau détecté");

      // Créer une notification d'erreur WordPress-style sous les boutons
      const notice = $(
        '<div class="notice notice-error is-dismissible"><p><strong>Erreur réseau :</strong> ' +
          errorMessage +
          "</p></div>"
      );
      $("#retrieve-cards-btn").after(notice);
